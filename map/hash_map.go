...
// Iterator returns an iterator for the map m..
...
// Put associates the specified value v with the specified key k in map m. If the key already exists then its value will be updated. It
...
// Len returns the size of the map m.
func (m *HashMap[K, V]) Len() int {
	return m.len
}

// Get retrieves the value associated with key k in map m. If there is no such value the zero value is returned along with false.
